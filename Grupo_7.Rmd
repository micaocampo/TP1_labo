...
```
  ...
  ¿Hay datos faltantes en el `data.frame`? Haciendo una tabla, vemos que no.
```{r, echo=FALSE}
...
####B. Relación entre matrícula y cantidad de secciones
  Para determinar las relaciones entre la matrícula y la cantidad de secciones discriminando por modalidad, área y sector, fueron omitidos los siguientes 3 casos donde la matrícula es mayor a 4000 por cuestiones estéticas a la visualización:
...
promedio_matricula_secciones <- datos %>%
...
```
  ...
  Nota: En este gráfico y en el cálculo del promedio no se omitió ninguna observación.
...
datos_modalidad_promedio <- datos %>%
  group_by(modalidad) %>%
  summarise(promedio = mean(matricula)/mean(secciones)) #Ver si se puede agregar desvio

ggplot(data = datos_modalidad_promedio, mapping = aes(x = modalidad, y = promedio, color = modalidad)) +
  geom_col(alpha = 0.7) +
  labs(
    title = "Promedios de cada modalidad con respecto a la matrícula y secciones",
    x = "Modalidad",
    y = "Promedio")

knitr::kable((datos_modalidad_promedio), caption = "Promedio por modalidad")
...
```{r, echo=FALSE}
ggplot(data = datos_filtrados_por_matricula, mapping = aes(x = matricula, y = secciones, color = area)) + 
...
```{r, echo=FALSE}
ggplot(data = datos_filtrados_por_matricula, mapping = aes(x = matricula, y = secciones, color = sector)) + 
  geom_point(alpha = 0.3) +
  labs(
